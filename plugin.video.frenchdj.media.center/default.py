...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(FMC, erro de codigo - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(FMC, erro de servidor. - "+str(e.reason)+",10000,"+icon+")")
...
def getSources():
        try:
            if os.path.exists(favorites) == True:
                addDir('[B][COLOR deepskyblue][ FMC Favoritos ][/COLOR][/B]','[B][COLOR deepskyblue][ FMC Favoritos ][/COLOR][/B]',4,'http://imgur.com/4z8iGDK.png' , os.path.join(home, 'fanart.jpg'),FANART,'','','','')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(FMC, nova fonte adicionada.,5000,"+icon+")")
        if not url is None:
...
def getSearchData(url,icon, data=None):
    ...
    keyboard.setHeading("[B][COLOR lime]Pesquisa de conteúdo[/B][/COLOR]")
    keyboard.setDefault('')
    keyboard.doModal()
    if keyboard.isConfirmed():
        ...
        xbmcgui.Dialog().ok('[B][COLOR lime]FMC[/B][/COLOR]', '[COLOR red][B]ATENÇÃO:[/B] [COLOR white]Pesquisas em branco não são autorizadas.[/COLOR]')
        quit()
    fanart=''
    dontLink=False
    os.path.join(home, 'resources', 'fanart.gif')
...
def playmediawithproxy(media_url, name, iconImage,proxyip,port, proxyuser=None, proxypass=None): #jairox
    ...
    if media_url==None or media_url=='':
        xbmc.executebuiltin("XBMC.Notification(FMC,Unable to play empty Url,5000,"+icon+")")
        return
        ...
        try:
            while player.is_active:
                ...
                if player.urlplayed==False and time.time()-beforestart>12:
                    print 'failed!!!'
                    xbmc.executebuiltin("XBMC.Notification(FMC,Unable to play check proxy,5000,"+icon+")")
                    break
                #xbmc.sleep(1000)
        except: pass
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(FMC,Urlresolver donot support this domain. - ,5000)")
        resolver=url
    return resolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('FMC','Escolhe um local para salvar os Videos.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('FMC', 'Queres adicionar este ficheiro como Fonte?')
        if ret:
...
params=get_params()
Base = 'aHR0cDovL2ZyZW5jaGRqLmF0c3BhY2UudHYvZm1jLnBocD91c2VyPQ=='.decode('base64')+USER+'JnBhc3M9'.decode('base64')+PASS
url=None
name=None
mode=None
playlist=None
...
#            ln+='</item>'
        ...
        if not (regexs and 'notplayable'  in regexs and not url):        
            if url:
                ...
                xbmc.executebuiltin("XBMC.Notification(FMC,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(FMC,Instala o [COLOR yellow]  Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(FMC,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(FMC,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(FMC,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
